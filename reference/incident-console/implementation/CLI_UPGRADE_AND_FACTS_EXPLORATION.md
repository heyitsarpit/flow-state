...
proposal and migration inventory; it does not amend `CLI.md`, close `BEH-033`,
or add a public API.
...
`contracts/PROOF_MATRIX.md`. `contracts/PERSISTENCE_AND_ARTIFACTS.md` changes
only if a new fact document becomes a persisted artifact. `BEH-033` must close
before freezing exact JSON fields, codecs, or diagnostic shapes.
