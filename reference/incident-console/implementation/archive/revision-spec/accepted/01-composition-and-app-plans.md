...
consumer state. A successful Persistence capture MUST capture only registered non-disposed stable actors
whose `actorRef(..., { persist: true })` declaration opts them in, including suspended stable actors, plus
the transitive closure of their exact context-provider refs. The closure MUST be computed from declaration
metadata, runtime registration, and fixed bindings, not automatic roots, child categories, current
projections, or whether the current factory called `ensureActor` during this runtime. Runtime-local actors,
runtime-incarnation tombstones, and disposed stable actors MUST NOT be captured. A restored stable actor
remains runtime-owned and captureable even when the current factory does not repeat `ensureActor` for it.
...
Persistence MUST fail terminally when an included persistable consumer has a direct or transitive context
dependency on an opaque local provider. Flow MUST NOT serialize, promote, recreate, substitute, or
rebind that provider automatically. `FlowPersistenceError` MUST add the non-retryable kind
...
**Proof obligations:** Prove `persist: false` defaults, declaration-owned membership for persistable stable
actors and operation families, suspended/disposed/local/tombstoned exclusion, exact transitive stable-provider
closure, factory omission not removing a restored actor, opaque-provider fail-closed diagnostics, and
dependency-ordered restoration with no context-event replay. Prove the context-closed revision cut and retry
...
`machine(...)` MUST own behavior; the app MUST close ownership and reachability; the runtime Implementation
...
restore-or-create boundary used by the production `Runtime` or another explicit runtime owner.
Only that production `Runtime` or an explicit runtime owner MAY invoke the ownership-capable ensure path;
...
disposal capability.
It MUST return an owner lease `{ actor, dispose }`. If Persistence restored the ref, `lease.actor` MUST be
...
handle from `Runtime.createActor`, and installs the method on every actor; migration MUST change all
...
shared actor resolved through `getActor` MUST remain owned by the production `Runtime`'s retained
lease and MUST NOT be disposed by the Story.

**Proof obligations:** Prove Persistence-restored runtime ownership, factory ensure joining, and freshly
...
incarnation under ordinary Persistence rules. Tombstones MUST NOT be persisted as actor snapshots and MUST
...
**Rule:** RuntimeSetup discovery MUST be synchronous and inert: it MAY retain app identity, Clock,
external capabilities, the optional Persistence provider, and its initial actor claims, but MUST NOT acquire
an Implementation, create or register actors, start work, or expose handles. Runtime readiness/bootstrap
MUST then be one atomic phase that validates the Persistence record, acquires the application Implementation,
validates and installs declared persistable actor claims, completes the `Runtime`'s initial
`ensureActor` calls, and resolves all exact context-provider refs. Missing providers, duplicate registrations,
foreign machines, and instance cycles MUST be rejected before the graph is sealed. Only after the graph is
sealed, the Implementation is available, and every attached actor crosses its activation barrier MAY any actor
activate, external operation begin, or public runtime or actor handle escape. Live hosts and app Stories MUST
cross the same production barrier; Stories MUST NOT use a testing-only registration or activation path.
...
**Proof obligations:** Prove inert discovery, exact bootstrap order, Implementation and provider failure,
