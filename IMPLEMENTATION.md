...
- [ ] Establish one Rust-like diagnostic convention for library-facing failures.
  ...
  - [x] Replace generic public/runtime/react/descriptors `throw new Error(...)` sites with tagged diagnostics or fail-closed defects that preserve `Cause`, relevant ids, and current runtime context.
    - The remaining raw `throw new Error(...)` guards now live in test-only assertion scaffolding; live descriptor, provider, runtime-service, resource-store, and view-selection paths route library-facing failures through `FlowDiagnostic` / `FlowBug` instead of generic exceptions, synchronous transaction `params`, `preview.apply`, `invalidates`, and `commit` callback throws now wrap into `FLOW-TXN-002`, synchronous transaction `routes.success|failure|defect|interrupt` callback throws now wrap into `FLOW-TXN-003`, synchronous stream `params`, `subscribe`, `pressure.key`, plus `routes.*` callback throws now wrap into `FLOW-STREAM-001`, and synchronous view `select` callback throws now wrap into `FLOW-VIEW-001`, all with `Schema.Defect({ includeStack: true })` debug payloads plus preserved raw `error.cause`.
  ...
  - [x] Reserve a separate `bug[...]` lane for invariant failures so impossible states do not look like expected user/runtime errors.
    - `packages/flow-state/src/diagnostics.ts` now holds the serializable diagnostic documents plus the stable formatter, while `react/provider.test.ts`, `app-inventory.test.ts`, `orchestrator-system.test.ts`, `resource-store.test.ts`, `transaction-callbacks.test.ts`, `transaction-outcome-callbacks.test.ts`, `stream-callbacks.test.ts`, `view-callbacks.test.ts`, `transactions.test.ts`, `runtime-streams.test.ts`, `flow-test-streams.test.ts`, `views.test.ts`, `react/use-view.test.ts`, and `diagnostics.test.ts` lock the first `FLOW-APP-*`, `FLOW-STORE-*`, `FLOW-ORCH-*`, `FLOW-TXN-*`, `FLOW-STREAM-*`, `FLOW-VIEW-*`, `FLOW-REACT-*`, and `bug[...]` exemplar surfaces without widening the package root API yet.
...
- [ ] Public diagnostics follow the tagged code/help convention with preserved `Cause` details and sourcemapped stacks.
  - Transaction setup callbacks, transaction outcome route callbacks, stream callbacks, and view select callbacks now preserve encoded `Cause` detail through `FLOW-TXN-002` / `FLOW-TXN-003` / `FLOW-STREAM-001` / `FLOW-VIEW-001` across the public `selectView(...)`, React `flow.useView(...)`, runtime actor, and `flowTest` surfaces, but other callback families still need the same tagged/sourcemapped treatment before this closeout item is honest.
