...
- [ ] Establish one Rust-like diagnostic convention for library-facing failures.
  ...
  - [x] Replace generic public/runtime/react/descriptors `throw new Error(...)` sites with tagged diagnostics or fail-closed defects that preserve `Cause`, relevant ids, and current runtime context.
    - The remaining raw `throw new Error(...)` guards now live in test-only assertion scaffolding; live descriptor, provider, runtime-service, resource-store, and view-selection paths route library-facing failures through `FlowDiagnostic` / `FlowBug` instead of generic exceptions, synchronous transaction `params`, `preview.apply`, `invalidates`, and `commit` callback throws now wrap into `FLOW-TXN-002`, synchronous transaction `routes.success|failure|defect|interrupt` callback throws now wrap into `FLOW-TXN-003`, synchronous machine `update` plus `actions.exit|transition|entry` callback throws now wrap into `FLOW-MACHINE-001`, synchronous stream `params`, `subscribe`, `pressure.key`, plus `routes.*` callback throws now wrap into `FLOW-STREAM-001`, and synchronous view `select` callback throws now wrap into `FLOW-VIEW-001`, all with `Schema.Defect({ includeStack: true })` debug payloads plus preserved raw `error.cause`.
