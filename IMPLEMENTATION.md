...
- [ ] Establish one Rust-like diagnostic convention for library-facing failures.
  ...
  - [x] Replace generic public/runtime/react/descriptors `throw new Error(...)` sites with tagged diagnostics or fail-closed defects that preserve `Cause`, relevant ids, and current runtime context.
    - The remaining raw `throw new Error(...)` guards now live in test-only assertion scaffolding; live descriptor, provider, runtime-service, resource-store, and view-selection paths route library-facing failures through `FlowDiagnostic` / `FlowBug` instead of generic exceptions, synchronous transaction `params`, `preview.apply`, `invalidates`, and `commit` callback throws now wrap into `FLOW-TXN-002`, synchronous transaction `routes.success|failure|defect|interrupt` callback throws now wrap into `FLOW-TXN-003`, synchronous machine `context`, `update`, plus `actions.exit|transition|entry` callback throws now wrap into `FLOW-MACHINE-001`, synchronous stream `params`, `subscribe`, `pressure.key`, plus `routes.*` callback throws now wrap into `FLOW-STREAM-001`, and synchronous view `select` callback throws now wrap into `FLOW-VIEW-001`, all with `Schema.Defect({ includeStack: true })` debug payloads plus preserved raw `error.cause`.
...
- [ ] Public diagnostics follow the tagged code/help convention with preserved `Cause` details and sourcemapped stacks.
  - Transaction setup callbacks, transaction outcome route callbacks, machine `context`, `update`, plus `actions.exit|transition|entry` callbacks, stream callbacks, and view select callbacks now preserve encoded `Cause` detail through `FLOW-TXN-002` / `FLOW-TXN-003` / `FLOW-MACHINE-001` / `FLOW-STREAM-001` / `FLOW-VIEW-001` across public snapshot creation, runtime actors, `flowTest`, and public view surfaces, but other callback families still need the same tagged/sourcemapped treatment before this closeout item is honest. Guard misses remain intentionally fail-closed rather than being reclassified as tagged callback defects.
