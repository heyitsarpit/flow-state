...
| `useActor`    | Read and drive a workflow actor from React.                           |
...
For the receipt-backed rationale behind `flow.module(...)`, `flow.app(...)`,
and `App.layer(...)`, read
...
| Import path          | Owns                                                           |
| -------------------- | -------------------------------------------------------------- |
| `flow-state`         | Core builders, keys, tags, runtime creation, shared types.     |
...
```ts
import { machine, resource, transaction } from "flow-state";
```
...
- The governing compatibility contract is `API_CONTRACT.md`;
  Launch Workspace's five-fact evidence matrix is maintained separately in
  `examples/launch-workspace/API_INVENTORY.md` and does not replace that
  contract.
- For handles, request boot, and inspection boundaries, use
  [Runtime](/reference/runtime) instead of treating this page as a second guide.
