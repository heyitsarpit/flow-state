...
type ReplacementInterleavingCase = Readonly<{
  readonly surface: "flowTest" | "runtime-actor";
  readonly policy: "allow" | "cancel-previous";
  readonly olderOutcome: "success" | "failure" | "defect";
  readonly completionOrder: "older-first" | "newer-first";
...
const replacementInterleavingCases: ReadonlyArray<ReplacementInterleavingCase> = (
  ["flowTest", "runtime-actor"] as const
).flatMap((surface) =>
  (["allow", "cancel-previous"] as const).flatMap((policy) =>
...
function replacementInterleavingOracle(caseDef: ReplacementInterleavingCase) {
  const transactionId =
    caseDef.policy === "allow"
      ? caseDef.olderOutcome === "defect"
        ? "transactions.save-allow-defect"
        : "transactions.save-allow"
      : caseDef.olderOutcome === "defect"
        ? "transactions.save-cancel-defect"
        : "transactions.save-cancel";

  return Object.freeze({
    transactionId,
    resourceId: "transactions.project",
    callNames: [caseDef.olderName, caseDef.newerName] as const,
    pending: Object.freeze({
      status: "pending" as const,
      resourceName: caseDef.newerName,
      interruptCount: caseDef.policy === "cancel-previous" ? 1 : 0,
    }),
    afterFirst:
...
function completeReplacementOlderAttempt(
  caseDef: ReplacementInterleavingCase,
  controls: ControlledSaveControls | ControlledSaveExitControls,
) {
  if (caseDef.olderOutcome === "success") {
    controls.succeedAt(0, { id: "project-1", name: caseDef.olderName });
    return;
  }

  if (caseDef.olderOutcome === "failure") {
    controls.failAt(0, "conflict");
    return;
  }

  if (isControlledSaveExitControls(controls)) {
    controls.defectAt(0, new Error("older replacement defect"));
    return;
  }

  throw new Error("Expected defect-capable controls for replacement interleaving oracle");
}

function expectReplacementInterleavingHarnessStage(
  harness: FlowTestHarness<
    SerialSaveContext | AllowDefectContext | CancelDefectContext,
    SerialSaveEvent | AllowDefectEvent | CancelDefectEvent,
    "ready"
  >,
  caseDef: ReplacementInterleavingCase,
  stage:
    | ReturnType<typeof replacementInterleavingOracle>["afterFirst"]
    | ReturnType<typeof replacementInterleavingOracle>["final"],
  expected: ReturnType<typeof replacementInterleavingOracle>,
) {
  expect(harness.context()).toMatchObject({
    savedNames: stage.savedNames,
    error: stage.error,
    ...(caseDef.olderOutcome === "defect" ? { defected: stage.defected } : {}),
  });
  expect(harness.issues()).toHaveLength(stage.issueCount);
  expect(harness.cache().query(expected.resourceId)).toMatchObject({
    value: { id: "project-1", name: stage.resourceName },
  });
  if (stage.status === "success") {
    expect(harness.transactions().get(expected.transactionId)).toMatchObject({
      status: stage.status,
      value: { id: "project-1", name: stage.valueName },
    });
  } else {
    expect(harness.transactions().get(expected.transactionId)).toMatchObject({
      status: stage.status,
  ...
  expect(
    harness
      ...
      .events(expected.transactionId)
      .filter((receipt) => receipt.type === "transaction:interrupt"),
  ).toHaveLength(stage.interruptCount);
  expect(
    harness
      .transactions()
      .events(expected.transactionId)
      .filter((receipt) => receipt.type === "transaction:success"),
  ).toHaveLength(stage.successCount);
  expect(
    harness
      .transactions()
      .events(expected.transactionId)
      .filter((receipt) => receipt.type === "transaction:failure"),
  ).toHaveLength(stage.failureCount);
  expect(
    harness
      .transactions()
      .events(expected.transactionId)
      .filter((receipt) => receipt.type === "transaction:defect"),
  ).toHaveLength(stage.defectCount);
}

type ReplacementInterleavingActorReadSurface = Readonly<{
...
function expectReplacementInterleavingRuntimeStage(
  actor: ReplacementInterleavingActorReadSurface,
  caseDef: ReplacementInterleavingCase,
  stage:
    | ReturnType<typeof replacementInterleavingOracle>["afterFirst"]
    | ReturnType<typeof replacementInterleavingOracle>["final"],
  ...
  expect(actor.issues()).toHaveLength(stage.issueCount);
  expect(actor.snapshot().resources[expected.resourceId]).toMatchObject({
    value: { id: "project-1", name: stage.resourceName },
  });
  if (stage.status === "success") {
  ...
  expect(
    actor
      ...
      .filter(
        (receipt) =>
          receipt.id === expected.transactionId && receipt.type === "transaction:failure",
      ),
  ).toHaveLength(stage.failureCount);
  expect(
    actor
      .receipts()
      .filter(
        (receipt) => receipt.id === expected.transactionId && receipt.type === "transaction:defect",
      ),
  ).toHaveLength(stage.defectCount);
}

async function expectReplacementInterleavingHarnessMatchesOracle(
  caseDef: ReplacementInterleavingCase,
) {
  const expected = replacementInterleavingOracle(caseDef);
  const controls =
    caseDef.olderOutcome === "defect"
  ...
  const machine =
    caseDef.policy === "allow"
      ...
      : caseDef.olderOutcome === "defect"
        ? cancelDefectMachine
        : cancelMachine;
  const harness = runSeededAppScenario(machine, {
    provide: controls.layer,
    events: [
      { type: "SAVE", name: caseDef.olderName },
      { type: "SAVE", name: caseDef.newerName },
  ...
  expect(controls.calls.map((params) => params.draft.name)).toEqual(expected.callNames);
  expect(harness.cache().query(expected.resourceId)).toMatchObject({
    value: { id: "project-1", name: expected.pending.resourceName },
  });
  expect(harness.transactions().get(expected.transactionId)).toMatchObject({
  ...
  expect(
    harness
      ...
      .events(expected.transactionId)
      .filter((receipt) => receipt.type === "transaction:interrupt"),
  ).toHaveLength(expected.pending.interruptCount);

  if (caseDef.completionOrder === "older-first") {
    completeReplacementOlderAttempt(caseDef, controls);
  } else {
    controls.succeedAt(1, { id: "project-1", name: caseDef.newerName });
  }
  await harness.flush();
  ...
  expectReplacementInterleavingHarnessStage(harness, caseDef, expected.afterFirst, expected);
  ...
  expectReplacementInterleavingHarnessStage(harness, caseDef, expected.final, expected);
  expectNoPendingWork(harness);
}

async function expectReplacementInterleavingRuntimeActorMatchesOracle(
  caseDef: ReplacementInterleavingCase,
) {
  const expected = replacementInterleavingOracle(caseDef);
  const controls =
    ...
    caseDef.olderOutcome === "defect"
      ? flow.runtime(
          flow
            .app({
              modules: [
                flow.module("TransactionsReplacementInterleaving", {
                  resources: {
                    project: projectResource,
                  },
                  transactions:
  ...
  try {
    ...
    expectReplacementInterleavingRuntimeStage(actor, caseDef, expected.afterFirst, expected);
  ...
  for (const caseDef of replacementInterleavingCases) {
    if (caseDef.surface === "flowTest") {
      it(`matches the generated replacement interleaving oracle in ${caseDef.surface} for ${caseDef.policy}, ${caseDef.olderOutcome}, and ${caseDef.completionOrder}`, async () => {
        await expectReplacementInterleavingHarnessMatchesOracle(caseDef);
      });
    }
  }
