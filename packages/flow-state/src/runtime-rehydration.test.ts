...
describe("runtime snapshot restoration", () => {
  ...
  it("rejects a restored scheduled timer whose persisted timer:start receipt omits owner identity fields", async () => {
    const machine = flow.machine<{}, never, "waiting" | "done">({
      id: "rehydration.invalid.timer-start-shape.machine",
      initial: "waiting",
      context: () => ({}),
      states: {
        waiting: {
          after: flow.after({
            id: "rehydration.invalid.timer-start-shape.after",
            delay: "1 second",
            target: "done",
          }),
        },
    ...
    const restoredSnapshot = Object.freeze({
      ...
      value: "waiting" as const,
      timers: {
        "rehydration.invalid.timer-start-shape.after": {
          id: "rehydration.invalid.timer-start-shape.after",
          status: "scheduled" as const,
          generation: 2,
          parentState: "waiting",
      ...
      receipts: [
        { type: "actor:start", id: "rehydration.invalid.timer-start-shape.actor" },
        {
          type: "timer:start",
          id: "rehydration.invalid.timer-start-shape.after",
          startedAt: 0,
    ...
    try {
      ...
      try {
        runtime.createActor(machine, {
          id: "rehydration.invalid.timer-start-shape.actor",
          snapshot: restoredSnapshot,
        });
      } catch (error) {
        restoreError = error;
      ...
      expect(restoreError).toMatchObject({
        code: "FLOW-TIMER-001",
        debug: {
          machineId: "rehydration.invalid.timer-start-shape.machine",
          timerId: "rehydration.invalid.timer-start-shape.after",
          parentState: "waiting",
          generation: 2,
          status: "scheduled",
          startedAt: 0,
          dueAt: 1_000,
          reason: "scheduled-timer-missing-start-receipt",
          allowedTimerIds: ["rehydration.invalid.timer-start-shape.after"],
        },
      });
      expect(runtime.orchestrators.get("rehydration.invalid.timer-start-shape.actor")).toBe(null);
    } finally {
      await runtime.dispose();
    }
  });
  ...
  it("rejects a restored scheduled timer whose persisted parentState does not match the restored state", async () => {
    const machine = flow.machine<{}, never, "waiting" | "paused" | "done">({
      ...
      states: {
        waiting: {
          after: flow.after({
            id: "rehydration.invalid.timer-parent-state.after",
            delay: "1 second",
            target: "done",
          }),
        },
    ...
    const restoredSnapshot = Object.freeze({
      ...
      value: "waiting" as const,
      timers: {
        "rehydration.invalid.timer-parent-state.after": {
          id: "rehydration.invalid.timer-parent-state.after",
          status: "scheduled" as const,
          generation: 2,
          parentState: "paused",
      ...
      receipts: [
        { type: "actor:start", id: "rehydration.invalid.timer-parent-state.actor" },
        {
          type: "timer:start",
          id: "rehydration.invalid.timer-parent-state.after",
          generation: 2,
    ...
    try {
      ...
      try {
        runtime.createActor(machine, {
          id: "rehydration.invalid.timer-parent-state.actor",
          snapshot: restoredSnapshot,
        });
      } catch (error) {
        restoreError = error;
      ...
      expect(restoreError).toMatchObject({
        code: "FLOW-TIMER-001",
        debug: {
          machineId: "rehydration.invalid.timer-parent-state.machine",
          timerId: "rehydration.invalid.timer-parent-state.after",
          parentState: "paused",
          ...
          allowedTimerIds: ["rehydration.invalid.timer-parent-state.after"],
        },
      });
      expect(runtime.orchestrators.get("rehydration.invalid.timer-parent-state.actor")).toBe(null);
    } finally {
      await runtime.dispose();
    }
  });

  it("rejects a restored scheduled timer whose persisted timer:start parentState does not match the timer snapshot", async () => {
    const machine = flow.machine<{}, never, "waiting" | "paused" | "done">({
      id: "rehydration.invalid.timer-start-parent-state.machine",
      initial: "waiting",
      context: () => ({}),
      states: {
        waiting: {
          after: flow.after({
            id: "rehydration.invalid.timer-start-parent-state.after",
            delay: "1 second",
            target: "done",
          }),
        },
    ...
    const restoredSnapshot = Object.freeze({
      ...
      value: "waiting" as const,
      timers: {
        "rehydration.invalid.timer-start-parent-state.after": {
          id: "rehydration.invalid.timer-start-parent-state.after",
          status: "scheduled" as const,
          generation: 2,
          parentState: "waiting",
      ...
      receipts: [
        { type: "actor:start", id: "rehydration.invalid.timer-start-parent-state.actor" },
        {
          type: "timer:start",
          id: "rehydration.invalid.timer-start-parent-state.after",
          generation: 2,
    ...
    try {
      ...
      try {
        runtime.createActor(machine, {
          id: "rehydration.invalid.timer-start-parent-state.actor",
          snapshot: restoredSnapshot,
        });
      } catch (error) {
        restoreError = error;
      ...
      expect(restoreError).toMatchObject({
        code: "FLOW-TIMER-001",
        debug: {
          machineId: "rehydration.invalid.timer-start-parent-state.machine",
          timerId: "rehydration.invalid.timer-start-parent-state.after",
          parentState: "waiting",
          ...
          allowedTimerIds: ["rehydration.invalid.timer-start-parent-state.after"],
        },
      });
      expect(runtime.orchestrators.get("rehydration.invalid.timer-start-parent-state.actor")).toBe(
        null,
      );
    } finally {
      await runtime.dispose();
  ...
  it("rejects a restored scheduled timer whose persisted timer:start generation does not match the timer snapshot", async () => {
    const machine = flow.machine<{}, never, "waiting" | "done">({
      id: "rehydration.invalid.timer-start-generation.machine",
      initial: "waiting",
      context: () => ({}),
      states: {
        waiting: {
          after: flow.after({
            id: "rehydration.invalid.timer-start-generation.after",
            delay: "1 second",
            target: "done",
          }),
        },
    ...
    const restoredSnapshot = Object.freeze({
      ...
      value: "waiting" as const,
      timers: {
        "rehydration.invalid.timer-start-generation.after": {
          id: "rehydration.invalid.timer-start-generation.after",
          status: "scheduled" as const,
          generation: 3,
          parentState: "waiting",
      ...
      receipts: [
        { type: "actor:start", id: "rehydration.invalid.timer-start-generation.actor" },
        {
          type: "timer:start",
          id: "rehydration.invalid.timer-start-generation.after",
          generation: 2,
    ...
    try {
      ...
      try {
        runtime.createActor(machine, {
          id: "rehydration.invalid.timer-start-generation.actor",
          snapshot: restoredSnapshot,
        });
      } catch (error) {
        restoreError = error;
      ...
      expect(restoreError).toMatchObject({
        code: "FLOW-TIMER-001",
        debug: {
          machineId: "rehydration.invalid.timer-start-generation.machine",
          timerId: "rehydration.invalid.timer-start-generation.after",
          parentState: "waiting",
          ...
          allowedTimerIds: ["rehydration.invalid.timer-start-generation.after"],
        },
      });
  ...
  it("rejects a restored scheduled timer whose persisted timer:start schedule facts do not match the timer snapshot", async () => {
    const machine = flow.machine<{}, never, "waiting" | "done">({
      id: "rehydration.invalid.timer-start-schedule-facts.machine",
      initial: "waiting",
      context: () => ({}),
      states: {
        waiting: {
          after: flow.after({
            id: "rehydration.invalid.timer-start-schedule-facts.after",
            delay: "1 second",
            target: "done",
          }),
        },
    ...
    const restoredSnapshot = Object.freeze({
      ...
      value: "waiting" as const,
      timers: {
        "rehydration.invalid.timer-start-schedule-facts.after": {
          id: "rehydration.invalid.timer-start-schedule-facts.after",
          status: "scheduled" as const,
          generation: 2,
          parentState: "waiting",
      ...
      receipts: [
        { type: "actor:start", id: "rehydration.invalid.timer-start-schedule-facts.actor" },
        {
          type: "timer:start",
          id: "rehydration.invalid.timer-start-schedule-facts.after",
          generation: 2,
    ...
    try {
      ...
      try {
        runtime.createActor(machine, {
          id: "rehydration.invalid.timer-start-schedule-facts.actor",
          snapshot: restoredSnapshot,
        });
      } catch (error) {
        restoreError = error;
      ...
      expect(restoreError).toMatchObject({
        code: "FLOW-TIMER-001",
        debug: {
          machineId: "rehydration.invalid.timer-start-schedule-facts.machine",
          timerId: "rehydration.invalid.timer-start-schedule-facts.after",
          parentState: "waiting",
          generation: 2,
          receiptParentState: "waiting",
          receiptGeneration: 2,
          ...
          allowedTimerIds: ["rehydration.invalid.timer-start-schedule-facts.after"],
        },
      });
      expect(
        runtime.orchestrators.get("rehydration.invalid.timer-start-schedule-facts.actor"),
      ).toBe(null);
    } finally {
      await runtime.dispose();
    }
