...
const foreignEventTransaction = flowCore.transaction<
  void,
  string,
  never,
  never,
...
const foreignEventStream = flowCore.stream<unknown, ForeignBindingEvent, void, string>({
...
flowCore.machine<{}, WorkspaceEvent, "idle">({
  ...
  states: {
    idle: {
      on: {
        // @ts-expect-error packed declarations reject foreign submitted event kinds
...
flowCore.machine<{}, WorkspaceEvent, "idle">({
  ...
  states: {
    idle: {
      // @ts-expect-error packed declarations reject foreign state-owned transaction event kinds
      invoke: flowCore.run(foreignEventTransaction),
    },
  },
});
flowCore.machine<{}, WorkspaceEvent, "idle">({
  ...
  states: {
    idle: {
      // @ts-expect-error packed declarations reject foreign state-owned stream event kinds
...
type MarkerlessForeignTransaction = WithoutRoutedEvent<
  flowCore.FlowTransactionBinding<ForeignBindingEvent>,
  ForeignBindingEvent
>;
...
flowCore.machine<{}, WorkspaceEvent, "idle">({
  ...
  states: {
    idle: {
      // @ts-expect-error packed declarations reject markerless transaction carriers
      invoke: flowCore.run(markerlessForeignTransaction),
    },
  },
});
flowCore.machine<{}, WorkspaceEvent, "idle">({
  ...
  states: {
    idle: {
      ...
      invoke: flowCore.run(reconstructedForeignTransaction),
    },
  },
});
...
flowCore.machine<{}, WorkspaceEvent, "idle">({
  ...
  states: {
    idle: {
      // @ts-expect-error packed declarations reject markerless stream carriers
...
export const workspaceInputChild = flowCore.child({
...
type _PackedChildInput = Expect<
  ...
  >
>;
const invalidWorkspaceChildInput = () => ({});
flowCore.child({
  id: "workspace.invalid-input",
  machine: workspaceChildMachine,
  // @ts-expect-error packed child input must construct the exact child context
  input: invalidWorkspaceChildInput,
});
