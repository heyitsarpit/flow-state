...
| `flow.transaction`        | Project save, Approval submit                                                            | Executable target authoring API with `params`, `commit`, `preview`, queued offline commits, undo rollback, and replay; currently runs through mutation receipts internally | Transaction and offline queue tests |
...
| Editor    | Project Editor           | Flow transitions, `flow.can`, typed save failure service test, editor view, preview rollback, offline save queue, undo, replay, conflict lane | `flow.ensure` and `flow.observe` live lookup execution                          |
...
- Offline project saves use transaction queue predicates. While offline, the
  transaction applies its preview patch and records `mutation:queue`; undo
  records `mutation:queue-drop` plus `mutation:rollback`; reconnect records
  `mutation:dequeue` and serializes queued commits in FIFO order.
