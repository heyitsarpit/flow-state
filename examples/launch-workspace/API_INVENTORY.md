...
| `flow.app`                | `LaunchWorkspaceApp`                                                                                                                 | Wired descriptor plus flattened app inventory, dependency/cycle validation, duplicate module-id validation, and duplicate resource-id validation; broader cross-module descriptor validation stays out of scope today | API and module inventory tests                   |
| `App.layer`               | `LaunchWorkspaceAppLayer`, `LaunchWorkspaceTestAppLayer`, `createLaunchWorkspaceRequestBoot`                                         | Wired Layer composition for the current live/test installer subset and request-scoped boot ownership through `withRequestRuntime(...)`                                                                                | API coverage test                                |
...
| `flow.ensure`             | Project editor loading                                                                                                               | Actor-owned ensure descriptor with runtime ResourceStore execution through the current supported resource command subset                                                                                              | API coverage and runtime tests                   |
| `flow.observe`            | Project editor comments observer                                                                                                     | Actor-owned observe descriptor with runtime ResourceStore execution and live observation ownership through the current supported subset                                                                               | API coverage and runtime tests                   |
| `flow.refresh`            | Project command contract                                                                                                             | Actor-owned refresh descriptor with runtime ResourceStore execution; broader cache TTL and refresh policy still remain partial                                                                                        | API coverage and runtime tests                   |
...
| `flow.invalidate`         | Readiness invalidation command                                                                                                       | Actor-owned invalidation by ref, tag, and filter is runtime-real; broader cache policy semantics remain partial                                                                                                       | API coverage and runtime tests                   |
...
| `flow.after`              | Assets complete dismissal                                                                                                            | Actor-owned one-shot timer descriptor with lifecycle snapshots and restored resume; broader recurring schedule policy remains future                                                                                  | Assets scenario and runtime tests                |
