...
| `flow.runtime`            | `createLaunchWorkspaceBrowserRuntime`, `createLaunchWorkspaceTestRuntime`, background Chat actor | Host bridge plus executable `runtime.resources` ResourceStore handle and `runtime.orchestrators` actor registry                                                                             | API coverage and app-layer tests                 |
...
| `flow.use`                | React shell flow subscription                                                                    | Executable provider-backed actor hook with render-safe shell creation, live actor swap, and unmount disposal                                                                                | `src/launchWorkspaceShell.tsx`                   |
