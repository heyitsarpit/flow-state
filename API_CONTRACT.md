...
- A child machine's input contract informs the parent input selector; the child
  implementation does not reshape the parent contract backwards.
...
| `flow.child({ id, machine, input, routes, supervision })`                             | Supervised child workflow                                    | Preserve helper; complete lifecycle and restore behavior                                           |
